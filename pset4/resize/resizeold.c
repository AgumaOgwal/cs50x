...
int main(int argc, char *argv[])
{
    ...
    int y = 0; //variable y to help loop through and put values into the array
    ...
    for (int i = 0, biHeight = abs(origbiHeight); i < biHeight; i++)
    {
        ...
        while(y<bi.biWidth)
        {
            for (int j = 0; j < origbiWidth; j++)
            {
                ...
                pixelArray[y] = triple;
                y++;
                //testing the changing values of Y
                //**************been giving errors************************//
                //so im commenting out
                 //fprintf(stderr, "Y is now %i\n and triple is %s", y,pixelArray[y]);
                }
        ...
        //****Rewrite Method Fails   
        // for(int m=0; m<size; m++)
        // {
        //     for(int k=0; k<sizeof(pixelArray); k++)
        //     {
        //         //fwrite(&pixelArray[k], sizeof(RGBTRIPLE), 1, outptr);
        //     }
        //     for(int x = 0; x < padding; x++)
        //     {
        //     fputc(0x00, outptr);
